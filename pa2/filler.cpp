...
template <template <class T> class OrderingStructure> animation filler::fill(FillerConfig &config)
{
    ...
    for(int i = 0; i< config.centers.size(); i++){
        ...
        while(!os.isEmpty()){
            ...
            neighbours.push_back(make_pair((temp.x)-1, temp.y));
            neighbours.push_back(make_pair(temp.x, (temp.y)+1));
            neighbours.push_back(make_pair((temp.x)+1, temp.y));
            neighbours.push_back(make_pair(temp.x, (temp.y)-1));

            for(int j = 0; j < 4; j++){
                float dist = sqrt(pow((int)neighbours.at(j).first-(int)config.centers.at(i).x,2) + 
                    pow((int)neighbours.at(j).second-(int)config.centers.at(i).y,2));
                    
                pair<int, int> c = neighbours.at(j);
                if(visited.count(c) == 0 && dist <= config.tolerance){
                    point* aux = new point(neighbours.at(j).first, neighbours.at(j).second, config.centers.at(i));
                    os.add(aux);
                    count++;
                }
            }

            *config.img.getPixel(temp.x, temp.y) = (*config.pickers.at(i))(temp);

            if(count%config.frameFreq == 0) output.addFrame(config.img);
            pair<int, int> n = make_pair(temp.x, temp.y);
            visited.insert(n);
        }
